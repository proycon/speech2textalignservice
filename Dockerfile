...
RUN cp /usr/src/webservice/runit.d/nginx.run.sh /etc/service/nginx/run &&\
    ...
    cp /usr/src/webservice/speech2textalignservice/speech2textalignservice.wsgi /etc/speech2textalignservice.wsgi &&\
    chmod a+x /etc/speech2textalignservice.wsgi &&\
    cp -f /usr/src/webservice/speech2textalignservice.nginx.conf /etc/nginx/sites-enabled/default
