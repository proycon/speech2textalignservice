#!/bin/sh
set -e
docker build -t "speech2textalignservice" .
...
#At this point you can pass any environment variables you use in your speech2textalignservice.config.yml, pass them via --env
docker run --rm --volume "$DATADIR:/data" -p "$HOST_PORT:80" --env CLAM_ROOT=/data/speech2textalignservice-userdata --env CLAM_USE_FORWARDED_HOST=0 "speech2textalignservice"
